...
| 4 | `step4_model_arima.ipynb` | Fit `ARIMAModel` via `run_single_its()`; inspect `FitResult` and pipeline outputs. |
| 4 | `step4_model_neuralprophet.ipynb` | Fit `NeuralProphetModel`; discuss AR warmup and its effect on the MBB residual pool. |
| 4 | `step4_model_prophet_xgb.ipynb` | Fit the Prophet + XGB **hybrid** (XGB on Prophet residuals). |
| 4 | `step4_model_prophet_then_xgb.ipynb` | Fit the Prophet-then-XGB **sequential** model (XGB corrects a standalone Prophet forecast). |
