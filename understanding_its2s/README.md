...
| 2 | `step2_cross_validation.ipynb` | `time_series_cv()`: expanding-window CV, fold layout, `skip_obs`, `cv_end_date` (leakage-safe by default, GH #40), and why temporal leakage matters. |
| 3 | `step3_hyperparameter_tuning.ipynb` | `tune_model()`: Latin hypercube search composed on top of step 2's CV. Demo on `prophet_xgb`; same mechanics apply to the other two models. |
...
The three step-4 notebooks are parallel variants; pick whichever architecture
...
`cross_validation.py` (non-overlapping fold layout, `skip_obs`,
`cv_end_date` with its leakage-safe derived default, GH #40). They are
explanatory and do not themselves exercise the
